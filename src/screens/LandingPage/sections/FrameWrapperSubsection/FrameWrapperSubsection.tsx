  ...
  return (
    ...
    >
      <div className="flex flex-col items-start gap-4 w-full">
        <h2 className="font-quicksand font-bold text-[#323232] text-[24px] leading-7 md:text-[28px] md:leading-8">
          Notre identité, notre vision
        </h2>
        <h3 className="font-mysticgrove font-normal text-[28px] leading-8 md:text-[32px] md:leading-9 tracking-[-0.5px]">
          <span className="text-[#323232]">Ce qui définit School-</span>
          <span className="text-[#3879F0]">UP</span>
        </h3>
        <p className="font-quicksand font-medium text-[#323232] text-[14px] leading-5 md:text-[16px] md:leading-6">
      ...
      <div className="w-full md:w-[1340px] rounded-[20px] shadow-[0_1px_4px_rgba(12,12,13,0.05)] inline-flex flex-col items-start"style={{ backgroundColor: activeTabData?.cardBg }}>
        <div className="self-stretch p-5 flex flex-col md:flex-row  gap-5">
          {tabsData.map((tab) => (
            <div
              key={tab.value}
              className={`self-stretch md:self-auto  p-5 rounded-[20px] flex items-center gap-2.5 cursor-pointer transition-colors ${tab.value === activeTab ? `bg-[${tab.activeBg}] text-[${tab.activeText}]`: " hover:text-[#3F3F46] hover:bg-transparent text-[#838383]"
              } ${tab.value === "engagement" ? "md:w-96" : "md:w-72"}`}
              onClick={() => setActiveTab(tab.value)}
              role="button"
              aria-label={tab.label}
            >
              <div className="self-stretch justify-start text-2xl font-normal font-['Krona_One'] leading-relaxed">
                {tab.label}
              </div>
            </div>
          ))}
