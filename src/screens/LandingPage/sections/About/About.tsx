...
export const FrameWrapperSubsection = (): JSX.Element => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`w-full  rounded-[20px] shadow-[0_1px_4px_rgba(12,12,13,0.05)]  inline-flex flex-col items-start ${
          activeTabData?.cardBgClass || "bg-[#E0E7FF]"
        }`}
      >
        <div className="self-stretch  p-5 flex flex-col md:flex-row md:flex-wrap md:justify-between  gap-5">
          {tabsData.map((tab) => (
            <div
              key={tab.value}
              className={`self-stretch flex-1 md:self-auto p-5 rounded-[20px] flex items-center gap-2.5 cursor-pointer transition-colors ${
                tab.value === activeTab
